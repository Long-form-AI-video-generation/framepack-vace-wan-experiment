...
class VaceImageProcessorLong(object):
...
class VaceVideoProcessorLong(object):
    ...
    def load_video_batch(self, *data_key_batch, crop_box=None, seed=2024, **kwargs):
        ...
        all_frames = [reader.get_batch(range(len(reader))) for reader in readers]
        print("Shape of all frames from first video:", all_frames[0].shape)
        chunk_size = 81
        stride = 3
        overlap = 16

        # Use only the first video for now
        video_frames = all_frames[0]  # shape: (T, H, W, C)

        frames_downsampled = video_frames[::stride]  # shape: (T/stride, H, W, C)
        chunks = []
        for i in range(0, frames_downsampled.shape[0] - chunk_size + 1, chunk_size - overlap):
            chunk = frames_downsampled[i:i + chunk_size]  # shape: (chunk_size, H, W, C)
            chunks.append(chunk)
