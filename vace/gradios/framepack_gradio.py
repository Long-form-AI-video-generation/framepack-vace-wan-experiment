...
class VACEInference:
    ...
    def create_ui(self, *args, **kwargs):
        gr.Markdown("""
                    <div style="text-align: center; font-size: 24px; font-weight: bold; margin-bottom: 15px;">
                        <a href="" style="text-decoration: none; color: inherit;">Framepack VACE-WAN Demo</a>
                    </div>
                    """)
    ...
    def generate(self, output_gallery, src_video, src_mask, src_ref_image_1, src_ref_image_2, src_ref_image_3, prompt, negative_prompt, shift_scale, sample_steps, context_scale, guide_scale, infer_seed, output_height, output_width, frame_rate, num_frames, progress=gr.Progress(track_tqdm=True)):
        ...
        video = self.pipe.generate_with_framepack(
    prompt,
    src_video,
    src_mask,
    src_ref_images,
    size=(output_width, output_height),  # Pass size as positional argument
