...
class WanVace(WanT2V):
    ...
    def generate_video_to_video(self,
                 input_prompt,
                 input_frames,
                 input_masks,
                 input_ref_images,
        ...
        if not self.t5_cpu:
            ...
            context = [t.to(self.device) for t in context]
            context_null = [t.to(self.device) for t in context_null]
        print('ref frames', input_frames.shape)
        for i, ref_img_set in enumerate(input_ref_images):
            if ref_img_set is None:
                print(f"  input_ref_images[{i}] is None")
            else:
                for j, ref_img in enumerate(ref_img_set):
                    if ref_img is None:
                        print(f"    input_ref_images[{i}][{j}] is None")
                    else:
                        print(f"    input_ref_images[{i}][{j}] shape: {ref_img.shape}")
        # vace context encode
        z0 = self.vace_encode_frames(input_frames, input_ref_images, masks=input_masks)
        m0 = self.vace_encode_masks(input_masks, input_ref_images)
        z = self.vace_latent(z0, m0)
